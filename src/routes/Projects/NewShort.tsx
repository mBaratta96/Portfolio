...
const scriptClasses = ["scene-heading", "dialogue", "character", "action", "parenthetical"];
...
const parseScript = (scriptString: string) => {
    ...
    return scriptClasses.reduce(
        (previous, current) => (classes[current] ? previous.replaceAll(`"${current}"`, classes[current]) : previous),
        script
    );
};

const scripts = [scene1Script, scene2Script, scene3Script].map((scriptString, index) => {
    const script = parseScript(scriptString);
    // add scene number in header
    const tempDiv = document.createElement("div");
    tempDiv.innerHTML = script;
    const sceneHeadings = tempDiv.querySelectorAll("[data-scene-heading-index]");
    sceneHeadings.forEach(
        (element) =>
            (element.textContent = element.getAttribute("data-scene-heading-index") + ". " + element.textContent)
    );
    return <div key={index} className={classes.script} dangerouslySetInnerHTML={{ __html: tempDiv.innerHTML }}></div>;
});

const content = sortBy(parse(contentString, { header: true }).data as newShortType[], ["index"]).map((data, index) => {
...
const title = (
    <Fragment>
        ...
        <a className={classes.link} target="_blank" rel="noreferrer noopener" href={scriptIta}>
            here
        </a>{" "}
        / English{" "}
        <a className={classes.link} target="_blank" rel="noreferrer noopener" href={scriptEng}>
            here
        </a>
        )
    </Fragment>
...
const NewShort = () => {
    return <Slide media={media} content={content} title={title} subtitle={subtitle} />;
};
